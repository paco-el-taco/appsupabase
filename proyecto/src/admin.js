...
export async function mostrarAdmin() {
  ...
  if (user.email !== "andresfelipelopezbravo6458@gmail.com") {
...
return;
        ...
        : `<ul>
            ${estudiantes
              ...
              <li style="margin-bottom:8px;">
                ...
                (${escapeHtml(est.correo)}) - ${escapeHtml(
  ...
  // Helper para normalizar relaciones
            ...
            ${actividades
              .map((act) => {
                ...
                const imagenHtml = act.imagen
                  ? `<div style="margin-top:6px;"><img src="${escapeAttr(
                      act.imagen
                    )}" alt="${escapeAttr(
                      act.titulo
                ...
                return `
                  <li style="margin-bottom:12px;border-bottom:1px solid #eee;padding-bottom:8px;">
                    ...
                    <div style="margin-top:6px;">
                      Nota: <input type="number" min="0" max="5" step="0.1"
                      value="${act.nota ?? ""}" data-id="${act.id}"
  ...
  document.querySelectorAll(".borrar-estudiante").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      if (
        !confirm(
          "¿Eliminar este estudiante? Esta acción también eliminará sus actividades (si la FK es cascade)."
        )
      )
        return;
